...
function StatsModels.fit(rng, t::Type{DRE}, data::AbstractArray{T,3}, designmatrix::AbstractArray{T,2}=similar(data, 0, 0); hidden_chs=3, kwargs...) where {T}
    ...
    ps, st = fit!(rng, dre, input_data, output_data; kwargs...)
...
function StatsModels.fit!(rng, dre::DRE, data_input, data_output; n_epochs=1, batch_size=32, kwargs...)
    ...
    ps, st = train(dre, data_input, data_output, ps, st; n_epochs=n_epochs, batch_size=batch_size)
    return ps, st
end